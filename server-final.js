/**
 * Final Autonomous Income Agent Server
 * Vollständige Version mit Content-Generierung und -Veröffentlichung
 */
...
let contentPublisher = null;

// Content-Speicher (in einer echten Anwendung wäre das eine Datenbank)
...
function initializeIntegrations() {
  ...
  if (configOverview.services.openai?.enabled) {
    const openaiConfig = config.getServiceConfig('openai');
    contentGenerator = new ContentGenerator(openaiConfig.apiKey, {
      model: openaiConfig.model,
      maxTokens: openaiConfig.maxTokens
    });
    console.log('✅ Content Generator initialisiert');
  }

  // Content Publisher
  const publisherConfig = {
    wordpress: config.getServiceConfig('wordpress'),
    twitter: config.getServiceConfig('social')?.twitter,
    linkedin: config.getServiceConfig('social')?.linkedin,
    mailchimp: config.getServiceConfig('email')?.mailchimp
  };
  
  contentPublisher = new ContentPublisher(publisherConfig);
  console.log('✅ Content Publisher initialisiert');
      ...
      if (gumroadOpportunities.success && gumroadOpportunities.opportunities.length > 0) {
        gumroadOpportunities.opportunities.forEach(opp => {
          newOpportunities.push({
            ...
            risk_score: Math.random() * 0.4 + 0.1,
            status: 'pending',
            source: 'gumroad_api',
            created_at: new Date().toISOString(),
            metadata: {
...
/**
 * Automatische Content-Generierung für genehmigte Opportunities
 */
async function generateContentForOpportunity(opportunity) {
  if (!contentGenerator) {
    console.log('⚠️ Content Generator nicht verfügbar');
    return { success: false, error: 'Content Generator nicht konfiguriert' };
  }

  try {
    console.log(`📝 Generiere Content für Opportunity: ${opportunity.title}`);
    
    const contentResult = await contentGenerator.generateBatchContent([opportunity], ['blog', 'social', 'email']);
    
    if (contentResult.success) {
      // Speichere generierten Content
      generatedContent[opportunity.id] = {
        opportunity,
        content: contentResult.generated_content[opportunity.id],
        generated_at: new Date().toISOString(),
        cost_estimate: contentResult.summary.estimated_cost
      };
      
      console.log(`✅ Content für Opportunity ${opportunity.id} generiert`);
      return { success: true, content: generatedContent[opportunity.id] };
    } else {
      console.error(`❌ Content-Generierung fehlgeschlagen:`, contentResult.errors);
      return { success: false, error: contentResult.errors.join(', ') };
    }
    
  } catch (error) {
    console.error(`❌ Fehler bei Content-Generierung für Opportunity ${opportunity.id}:`, error);
    return { success: false, error: error.message };
  }
}

/**
 * Automatische Content-Veröffentlichung
 */
async function publishContentForOpportunity(opportunityId, platforms = []) {
  const content = generatedContent[opportunityId];
  
  if (!content) {
    return { success: false, error: 'Kein Content für diese Opportunity verfügbar' };
  }

  if (!contentPublisher) {
    return { success: false, error: 'Content Publisher nicht verfügbar' };
  }

  try {
    console.log(`📤 Veröffentliche Content für Opportunity: ${content.opportunity.title}`);
    
    const publishResult = await contentPublisher.publishToMultiplePlatforms(
      content.content, 
      platforms
    );
    
    if (publishResult.success || publishResult.summary.successful_publications > 0) {
      // Speichere Veröffentlichungsdetails
      publishedContent[opportunityId] = {
        opportunity_id: opportunityId,
        publications: publishResult.publications,
        published_at: new Date().toISOString(),
        success_rate: publishResult.summary.successful_publications / publishResult.summary.total_platforms
      };
      
      console.log(`✅ Content für Opportunity ${opportunityId} veröffentlicht auf ${publishResult.summary.successful_publications} Plattformen`);
      return { success: true, result: publishResult };
    } else {
      console.error(`❌ Content-Veröffentlichung fehlgeschlagen:`, publishResult.errors);
      return { success: false, error: publishResult.errors.join(', ') };
    }
    
  } catch (error) {
    console.error(`❌ Fehler bei Content-Veröffentlichung für Opportunity ${opportunityId}:`, error);
    return { success: false, error: error.message };
  }
}
...
function startAgent() {
  ...
  const now = new Date();
  nextScan = new Date(now.getTime() + 4 * 60 * 60 * 1000);

  console.log('🚀 Enhanced Autonomous Income Agent gestartet');
    ...
    switch (req.params.service) {
      ...
      case 'openai':
        if (contentGenerator) {
          ...
          const testResult = await contentGenerator.generateBlogArticle({
            id: 'test',
            title: 'Test Opportunity',
            description: 'Test Beschreibung',
            category: 'test'
          }, { length: 'kurz (200 Wörter)' });
          
          result = {
            success: testResult.success,
            message: testResult.success ? 'OpenAI API erfolgreich getestet' : testResult.error,
            cost_estimate: testResult.metadata?.cost_estimate
          };
        } else {
          result = { success: false, message: 'OpenAI Integration nicht konfiguriert' };
        }
        break;
...
/**
 * Content für eine Opportunity generieren
 */
app.post('/api/opportunities/:id/generate-content', async (req, res) => {
  try {
    ...
    const result = await generateContentForOpportunity(opportunity);
    res.json(result);
...
/**
 * Content für eine Opportunity veröffentlichen
 */
app.post('/api/opportunities/:id/publish-content', async (req, res) => {
  try {
    const opportunityId = parseInt(req.params.id);
    const { platforms = [] } = req.body;
    
    const result = await publishContentForOpportunity(opportunityId, platforms);
    res.json(result);
...
app.get('/api/content/generated', (req, res) => {
  ...
  if (opportunity_id) {
    const content = generatedContent[parseInt(opportunity_id)];
    if (!content) {
      return res.status(404).json({ error: 'Kein Content für diese Opportunity gefunden' });
    }
    res.json(content);
  } else {
    res.json({
      total_content: Object.keys(generatedContent).length,
...
/**
 * Veröffentlichten Content abrufen
 */
app.get('/api/content/published', (req, res) => {
  const { opportunity_id } = req.query;
  
  if (opportunity_id) {
    const content = publishedContent[parseInt(opportunity_id)];
    if (!content) {
      return res.status(404).json({ error: 'Kein veröffentlichter Content für diese Opportunity gefunden' });
    }
    res.json(content);
  } else {
    res.json({
      total_published: Object.keys(publishedContent).length,
...
/**
 * Opportunity genehmigen (mit automatischer Content-Generierung)
 */
app.post('/api/opportunities/:id/approve', async (req, res) => {
  const opportunityId = parseInt(req.params.id);
  const { auto_generate_content = true, auto_publish = false, publish_platforms = [] } = req.body;
  const opportunity = opportunities.find(opp => opp.id === opportunityId);
  ...
  try {
    ...
    if (auto_generate_content && contentGenerator) {
      console.log(`🤖 Starte automatische Content-Generierung für: ${opportunity.title}`);
      contentResult = await generateContentForOpportunity(opportunity);
      
      // Automatische Veröffentlichung
      if (auto_publish && contentResult.success && publish_platforms.length > 0) {
        console.log(`📤 Starte automatische Veröffentlichung auf ${publish_platforms.length} Plattformen`);
        publishResult = await publishContentForOpportunity(opportunityId, publish_platforms);
      }
    }
    ...
    res.json({
      ...
      content_published: !!publishResult?.success,
      automation_results: {
        content_generation: contentResult,
        content_publishing: publishResult
      }
    });
...
app.get('/api/agent/status', (req, res) => {
  ...
  res.json({
    ...
    contentStats: {
      generated: Object.keys(generatedContent).length,
      published: Object.keys(publishedContent).length
    },
    integrations: {
      gumroad: !!gumroadIntegration,
      affiliate: !!affiliateIntegration,
...
app.get('/api/stats', async (req, res) => {
  try {
    ...
    // Content-Statistiken
    const contentStats = {
      total_generated: Object.keys(generatedContent).length,
      total_published: Object.keys(publishedContent).length,
      total_cost_estimate: Object.values(generatedContent).reduce((sum, content) => 
        sum + parseFloat(content.cost_estimate || 0), 0
      )
    };
    
    // Plattform-Performance
    const platformStats = {};
    Object.values(publishedContent).forEach(content => {
      content.publications.forEach(pub => {
        if (!platformStats[pub.platform]) {
          platformStats[pub.platform] = { total: 0, successful: 0 };
        }
        platformStats[pub.platform].total++;
        if (pub.success) {
          platformStats[pub.platform].successful++;
        }
      });
    });
...
// Server starten
app.listen(PORT, () => {
  console.log(`🚀 Final Autonomous Income Agent Server läuft auf Port ${PORT}`);
  ...
  console.log('  POST /api/opportunities/:id/generate-content - Content generieren');
  console.log('  POST /api/opportunities/:id/publish-content - Content veröffentlichen');
  console.log('  GET  /api/content/generated - Generierten Content abrufen');
  console.log('  GET  /api/content/published - Veröffentlichten Content abrufen');
